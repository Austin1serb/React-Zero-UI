...
import { ZeroTooltip } from './ZeroUiToolTip';
import { BentoItem, BentoHeader, BentoIcon, BentoTitle, BentoDescription, BentoBody } from './bento';
import { P3ColorsIcon } from './p3-colors-icon';

function Code({ children }: { children: ReactNode }) {
	return <div className="font-mono text-[13px]/7 text-gray-950 dark:text-white">{children}</div>;
}

export const ColorPicker: React.FC<{ zeroUi?: boolean }> = ({ zeroUi = false }) => {
	return (
					...
					return (
						<div className="mt-8 -mr-16 -mb-16 sm:mt-0">
							<div
								...
								style={{ '--sin': sin, '--cos': cos, '--angle': `${angle}deg`, '--columns': colors.length + 1 } as CSSProperties}>
								<div />
								{colors.map(color => {
									return (
										<div key={color}>
								...
								{shades.map((shade, shadeIdx) => {
									return (
										<Fragment key={shade}>
											...
											{colors.map(color => {
												let value = colorValues[`${color}-${shade}` as keyof typeof colorValues];
												return zeroUi ? (
													<ZeroTooltip
														key={color}
														tooltip={value}
													...
													<Tooltip
														...
														<TooltipTrigger>
															<div
																style={{ '--color': value, backgroundColor: value } as CSSProperties}
																className="h-(--height) w-(--width) inset-ring inset-ring-gray-950/10 transition-opacity group-hover:opacity-75 hover:opacity-100 dark:inset-ring-white/10"
															/>
														</TooltipTrigger>
														<TooltipPanel
															as="div"
